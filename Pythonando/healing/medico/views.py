...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Você já tem cadastro médico!')
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == 'GET':
        consulta = Consulta.objects.get(id=id_consulta)######## linhas iguais, poderia ficar uma só antes de if
